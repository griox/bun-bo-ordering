...
# Ensure current directory is in path for core import
sys.path.append(str(Path(__file__).parent))

try:
    from core import search, DATA_DIR  # type: ignore
except ImportError:
    # Fallback for different environments
    def search(query: str, domain: str = "style", max_results: int = 3) -> dict:
        return {"results": [], "count": 0, "domain": domain, "query": query}
    DATA_DIR = Path(".")
...
class DesignSystemGenerator:
    ...
    def _multi_domain_search(self, query: str, style_priority: Optional[List[str]] = None) -> Dict[str, Any]:
        """Execute searches across multiple domains."""
        results = {}
        for domain, config in SEARCH_CONFIG.items():
            if domain == "style" and style_priority:
                # For style, also search with priority keywords
                p_list = style_priority
                # Avoid slicing which causes Pyre2 errors in some environments
                priority_query = " ".join(p_list[i] for i in range(min(len(p_list), 2)))  # type: ignore
                combined_query = f"{query} {priority_query}"
                results[domain] = search(combined_query, domain, config["max_results"])  # type: ignore
            else:
                results[domain] = search(query, domain, config["max_results"])  # type: ignore
        return results
...
def format_ascii_box(design_system: dict) -> str:
    ...
    def wrap_text(text: str, prefix: str, width: int) -> List[str]:
        ...
        for word in words:
            # Ensure word is treated as string
            word_str = str(word)
            if len(current_line) + len(word_str) + 1 <= width - 2:
                space = " " if current_line != prefix else ""
                current_line = f"{current_line}{space}{word_str}"
            else:
                if current_line != prefix:
                    lines.append(current_line)
                current_line = f"{prefix}{word_str}"
...
# ============ MAIN ENTRY POINT ============
def generate_design_system(query: str, project_name: Optional[str] = None, output_format: str = "ascii") -> str:
    """
