...
import { ShoppingCart, Loader2, Minus, Plus, X } from 'lucide-react';
...
export function CartModal() {
    const { cart, getCartTotal, updateQuantity, removeFromCart, setSession, session } = useOrderStore();
    ...
    const handleConfirm = async () => {
        try {
            ...
            setIsOpen(false);
            setSession(null); // Clear session after successful order
        } catch (error) {
